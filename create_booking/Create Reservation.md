  ...
  "paths": {
    "/pms/reservations": {
      "post": {
        ...
        "description": "This endpoint can add reservation based on the data.\nThere are a lot of events that happen while creating a reservation!\n## Unique Contact\nYou need at lease one unique identifier for a contact. Email or Phone number. A reservation request tries to “Find or New” a contact. If there is an overlap with both email / phone unique contact keys the system cannot determine which existing contact matches your  request. See Duplicate Contact below.\nContacts will attempt be matched to an existing contact in this order and will match the first one found... `contact` array attributes match against existing contact to prevent duplicates in the system - id - primaryEmail - secondaryEmail - cellPhone - homePhone - otherPhone - proxyEmail\n\n*Warning:* If the system finds a contact that matches, it will then update all attributes passed in the reservation to match. In some cases this will cause one or more of the unique contact attributes to match a second Contact in the system and will fail due to duplicate unique keys.\nThese attributes validate as needing to be unique:\n\n  - Email Addresses checked to be unique compared to ALL of these\n    - primary email\n    - alternate email\n  - Phone numbers checked to be unique compared to ALL of these\n    - home phone\n    - cell phone\n    - other phone\n\n### Duplicate Contact\nIf you want to handle contacts in a different way, you will need to find the existing contact using the CRM Contacts API which is restricted to Server API Keys. With this API you can find/modify existing Contacts as you deem fit and then pass the id as `contactId` to a reservation.\n\n*Example:* _Reservation contact matches on more than one contact in PMS/CRM_\n- search for contact with same email - compare names and use this contact if it matches names - add notes to the reservation about the duplicate phone\n  - the admin should follow up by checking contact and possibly merging them in the system to prevent future duplications\n\nIf name does not match try the same check for phone number with matching name.\n## Reservation statuses\n#### reservation mode = Request\n- if Payment is included:\n    - Checks runs $1 charge which is refunded in order to authorize card set to Hold.\n    - if Auth fails, reservation fails\n- else if NO Payment method:\n    - Just set to Hold.\n\n_Note:_ can add a payment method later via server keys or GUI\n#### reservation mode = Confirmed\n- Must have a payment method\n    - Channel Keys: Only allow Electronic Payment Methods (Credit and E-Check)\n    - Server Keys: Can use Electronic, Cash, Check, & Other Payment Methods\n- amount details and other restrictions are defined on the attributes - Breakdown Reservations with Server Keys are created as Confirmed.\n",
        "requestBody": {
          "content": {
            "application/json": {
              "schema": {
                "title": "reservation.request.v1",
                "type": "object",
                "properties": {
                  "unitId": {
                    "type": "integer"
                  ...
                  "breakdown": {
                    "description": "Do not use this unless absolutely necessary. It will overriding the automated calculation in TRACK.\n\nAllowed for Server Keys,\nAlso, can be sent when custom pricing is enabled for Channel Keys.\n\nPricing is being defined by Channel or Server.\nThis will override the automation on TRACK for calculating reservation fees, and rates.\nTaxes are calculated off of the reservation dates and tax settings in the system.\n",
                    "type": "object",
                    "properties": {
                      "rates": {
                        "type": "array",
                      ...
                      "occupantRates": {
                        ...
                        "items": {
                          ...
                          "properties": {
                            "id": {
                              "type": "integer",
                              "description": "can be set to the occupant type id or be one of adults, children or pet"
                            },
                            "occupancy": {
                              "type": "integer",
                              "description": "Include total occupancy of that type"
                            },
                            "extraOccupancy": {
                              "type": "integer",
                              "description": "Optional - Include extra occupancy beyond what is included in the rate type"
                            },
                            "unitRate": {
                              "type": "number",
                              "format": "float",
                              "description": "Optional - Per night, per person rate"
                            },
                            "totalRate": {
                              "type": "number",
                              "format": "float",
                              "description": "Optional - Total rate = $nights * $extraOccupancy * $unitRate"
                            }
                          },
                          "required": [
                            "id",
                      ...
                      "fees": {
                        ...
                        "items": {
                          ...
                          "properties": {
                            "id": {
                              "type": "integer",
                              "description": "Fee id, based on available fee ids from pricing"
                            },
                            "quantity": {
                              "type": "integer",
                              "description": "Assumed 1 if not provided"
                            },
                            "unitRate": {
                              "type": "number",
                              "format": "float",
                              "description": "Rate for a single item"
                            },
                            "rate": {
                              "type": "number",
                              "format": "float",
                      ...
                      "total": {
                        "type": "number",
                        "format": "float",
                        "description": "Provide channel total here and TRACK will calculate the `estimatedTaxDiscrepancy` based on any difference in the tax calculations."
                      }
                    }
                  },
                  "occupants": {
                    "type": "object",
                    "description": "only available if not using breakdown, string (adult, children, pets) or integer ID for occupant type",
                    "additionalProperties": {
                      "oneOf": [
                        {
                          "type": "string",
                  ...
                  "contactId": {
                    "type": "integer",
                    "description": "required contactId or contact object"
                  },
                  "contact": {
                    "type": "object",
                    "description": "At least one of the following is required:\n- cellPhone\n- homePhone\n- otherPhone\n- primaryEmail\n- secondaryEmail",
                    "properties": {
                      "id": {
                        "type": "integer",
                        "description": "optionally use the contact id to update the contact information"
                      },
                      "firstName": {
                        "type": "string",
                        "description": "First Name",
                  ...
                  "addOns": {
                    "type": "array",
                    "description": "only available if not using breakdown",
                    "items": {
                      "type": "object",
                      "properties": {
                        "id": {
                  ...
                  "confirmationNumber": {
                    "description": "save an alternate confirmation number for the reservation\nthis is locked down for channel reservations so it cannot be changed from the PMS after the reservation is created\nUse `/api/pms/reservations/:reservation_id/altconf` to change via API\n",
                    "type": "string"
                  },
                  "travelInsurance": {
                    "type": "boolean",
                    "description": "Unit must have an insurance provider\n"
                  },
                  "notes": {
                    "type": "array",
                    "items": {
                  ...
                  "payment": {
                    "description": "Four payment methods are supported, but availability will depend on which type of key is used and customer options\n\nIf a payment, or payment method is required by the guarantee policy,\nit is required to be provided by the request in order to create a reservation.\n\nTesting data for payment accounts - https://docs.spreedly.com/reference/test-data/\n\n## If you provide an amount of 0 then it will just process as a HOLD\n\nYou can also not send payment information if you are doing a hold\n\n```\n\"payment\": {\n\n  \"amount\": 0\n\n},\n```\n\n\nIf you send payment information and your Guarantee Policy is not HOLD it will save the payment method to the folio\n\n```\n\"payment\": {\n\n  \"amount\": 0,\n\n  \"bankAccount\": {\n\n    \"accountNumber\": \"9876543210\",\n\n    \"routingNumber\": \"021000021\",\n\n    \"accountType\": \"personal-checking\",\n\n    \"saveAccount\": true\n\n  }\n\n},\n```\n",
                    "type": "object",
                    "properties": {
                      "amount": {
                        "type": "number",
                        "format": "float",
                        "description": "The amount should NOT be provided unless absolutely necessary.\nTRACK automatically calculates fees and required payments based on many settings.\nOnly provide this if - the payment amount is different than the deposit - if a payment should be charged regardless of mode - if you want to force a hold, pass `{amount: 0}`\nNote: If the specified `amount` does not match or exceed the guarantee policy, then it will be set to HOLD instead of CONFIRMED.\nCustom Price option must be defined for channel keys to use this option,\n  otherwise this amount will be ignored.\n"
                      },
                      "paymentCard": {
                        "description": "All Contexts, Credit card data can only be provided by vendors whom are PCI compliant.",
                        "type": "object",
                        "properties": {
                          "cardNumber": {
                            "type": "string",
                          ...
                          "cardExp": {
                            "type": "string",
                            "pattern": "^(0|1)([0-9]{1})[-\\/]([0-9]{2,4})$",
                            "description": "Date as MM-YY or MM-YYYY"
                          },
                          "cardCvv": {
                            "type": "string"
                          },
                          ...
                          "isVirtual": {
                            "type": "boolean",
                            "nullable": true,
                            "description": "this will automatically set the policy to \\#2 *Credit Card Guarantee*"
                          },
                          "effectiveDate": {
                            "type": "string",
                            "format": "date",
                          ...
                          "limit": {
                            "type": "number",
                            "nullable": true,
                            "description": "Limit for the virtual card charge"
                          },
                          "saveCard": {
                            "type": "boolean",
                            "description": "true to ensure saving card,"
                          }
                        }
                      },
                      "bankAccount": {
                        "description": "Optional payment method, All Contexts",
                        "type": "object",
                        "properties": {
                          "routingNumber": {
                            "type": "string",
                      ...
                      "netCommission": {
                        "description": "Server context and specfic channels",
                        "type": "object",
                        "nullable": true,
                        "properties": {
                          "commission": {
                          ...
                          "date": {
                            "description": "Optional - Payout date, defaults to arrival date, Date in ISO-8601 format",
                            "nullable": true,
                            "type": "string",
                            "format": "date"
                          },
                          "reference": {
                            "description": "Optional - Tracking reference for payment",
                            "type": "integer",
                            "nullable": true
                          }
                        }
                      ...
                      "paymentMethod": {
                        "description": "use existing payment method Option limited to server context",
                        "type": "object",
                        "properties": {
                          "id": {
                            "type": "integer",
                          ...
                          "reference": {
                            "type": "string",
                            "description": "Tracking reference for payment"
                          }
                        }
                      }
                    }
                  ...
                  "isTaxable": {
                    "description": "set to false if tax exempt",
                    "type": "boolean",
                    "default": true
                  },
                  "campaignId": {
                    "type": "integer",
                    "nullable": true,
                    "description": "ID overwrites a Token"
                  },
                  "campaignToken": {
                    "type": "string",
                    "description": "can use token if ID is not supplied"
                  },
                  "status": {
                    "type": "string",
                    "enum": [
                    ...
                    "default": "Hold",
                    "description": "### _Overrides Reservation Status_\n##### _use with EXTREME caution_\n\nThis attribute can be used to force a reservation back to Hold from Confirmed.\n\n*Example:* New breakdown reservations are created as Confirmed.\n    You may use this `status` attribute to switch these to Hold.\n\n##### _CAUTION_\n- Reservation Confirmed Triggers:\n  - `breakdown` reservation will save as Confirmed\n  - This will cause the Confirmed Reservation triggers to fire\n  - `status` will then change it to the specified status\n- Reservation Status Change\n  - After the reservation is created it will be updated to the provided status.\n  - The status change will NOT trigger additional events\n  - This is an *override* of the standard functionality.\n"
                  },
                  "subChannel": {
                    "type": "string",
                    "nullable": true,
